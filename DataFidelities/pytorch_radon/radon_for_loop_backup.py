...
class IRadon(nn.Module):
    ...
    def forward(self, x):
        ...
        # loss = torch.mean(reco + 1)
        # loss.backward()
        # exit()
        # self.all_grids = self.all_grids.to(x.device)
        for i_theta in range(len(self.theta)):
            reco += grid_sample(x, self.all_grids[i_theta].repeat(reco.shape[0], 1, 1, 1).to(x.device))

        # print(self.all_grids[1])
        # loss = torch.mean(grid_sample(x, self.all_grids[1].repeat(reco.shape[0], 1, 1, 1).to(x.device)))
        # loss.backward()
        # exit()
        if not self.circle:
            W = self.in_size
