...
from DataFidelities.BlurClass import BlurClass
...
class URED(nn.Module):
    ...
    def __init__(self, dObj, dnn: nn.Module, config:dict):
        ...
        self.tau = nn.Parameter(torch.tensor(config['tau_inti'], dtype=torch.float32))

    def denoise(self, n_ipt, create_graph=True, strict=True):
        denoiser = self.dnn(n_ipt, create_graph, strict)
        return denoiser

    def forward(self, n_ipt, n_y, bk, bkt, create_graph=False, strict=False, gt=None):
        tau = self.tau if self.tau - 0.28 <= 0 else 0.28
        delta_g = self.dObj.grad(n_ipt, n_y, bk, bkt)
        xSubD    = tau * (self.dnn(n_ipt, create_graph, strict))
...
class DEQFixedPoint(nn.Module):
    ...
    def forward(self, n_y, gt, create_graph=True, strict=True):
        tauList, gammaList,n_ipt_periter = None, None, None
        bk,bkt = self.f.dObj.init(gt)
        # compute forward pass and re-engage autograd tape
        z, self.forward_res, _ = self.solver_img(lambda z : self.f(z, n_y, bk,bkt, create_graph=False, strict=False, gt=gt), n_y, max_iter=100)
        z =  self.f(z.detach(), n_y, bk,bkt, create_graph=True, strict=True, gt=gt)
        ...
        if create_graph:
            ...
            def backward_hook(grad):
                # print(grad.shape)
                fTg = lambda y : y - self.f.gamma*self.f.dObj.fwd_bwd(y, bk, bkt)-autograd.grad(r0, z0, y, retain_graph=True)[0] + grad #
                g, self.backward_res, _ = self.solver_grad(fTg, grad, max_iter=50, **self.kwargs)
                return g
            z.register_hook(backward_hook)
        return z, self.f.tau, gammaList, n_ipt_periter, None#, loss_fp_dist
...
class sgdnetBlur(torch.nn.Module):
    """Train SGD-Net/Bach-Unfold with pixel loss"""
    def __init__(self, config, device):
        super(sgdnetBlur, self).__init__()
    ...
    def init_state(self, train_set=None, valid_set=None, test_set=None):
        ...
        if test_set is not None:
            ...
            train_dataset = TrainDataset(train_set['train_y'],train_set['train_gdt'])
            self.train_dataLoader = DataLoader(train_dataset, 
            batch_size=self.config['training'].batch_size, shuffle=True)
            valid_dataset = ValidDataset(valid_set['valid_y'], valid_set['valid_gdt'])
    ...
    def define_model(self):
        ...
        if self.set_mode == 'test':
            ...
            if self.config['warm_up']:
                self.load_warmup()
        dObj = BlurClass(fwd_set,  emParams=self.emParams)
        self.URED = URED(dObj, self.dnn, self.config['unfold_model'])
        self.network =  DEQFixedPoint(self.URED, nesterov, anderson, tol=1e-5).to(self.device)
        if self.config['multiGPU']:
            gpu_ids = self.config['GPU_index']
    ...
    def define_scheduler(self):
        ...
        self.scheduler = torch.optim.lr_scheduler.MultiStepLR(self.optimizer, milestones=[8], gamma=1e-4/5e-4)
    # ----------------------------------------
    ...
    def train_optimize(self):
        ...
        for epoch in range(1, self.opt_train['unfold_epoch']):
            ...
            for batch_y, batch_gdt in tqdm(self.train_dataLoader):
                batch_gdt = batch_gdt.to(self.device)
                batch_y = batch_y.to(self.device)
                self.optimizer.zero_grad()
                batch_pre, tau, _, _,_ = self.network(batch_y, batch_gdt, create_graph=False, strict=False)
                ...
                self.writer.add_scalar('train/tau',tau[0], self.global_step)
                loss.backward()
            ...
            snr_avg = 0
            count = 0
            for batch_y, batch_gdt in self.valid_dataLoader:
                batch_gdt = batch_gdt.to(self.device)
                batch_y = batch_y.to(self.device)
                batch_pre, _, _, _, _ = self.network(batch_y, batch_gdt, create_graph=False)
            ...
            Img_ipt = utils.make_grid(batch_y, nrow=4, normalize=False, scale_each=True)
